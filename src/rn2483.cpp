/*!
    ...
    Modified to fit this USV logging project (read / write)
...
//! read a single byte from RN2483 via UART
uint8_t read()
{
    lora_request_t* req = get_curr_request();
    if (!req) return '\n'; // safety check
    ...
    if (req->id != current_id)
    {
        ...
        if (req->lora_rx_len == 0)
            return '\n';

        return req->lora_rx_buf[current_index++];
    }
    else
    {
        // give the next byte in the array
        if (current_index >= req->lora_rx_len) return '\n';
        return req->lora_rx_buf[current_index++];
    }
}
...
{
    write_uart(string, strlen(string));
}
